...
Deliberately NOT ported: Claude Code's `grt` step, which prepends a full
...
INSTRUCTIONS_BODY = """Your task is to create a detailed summary of the conversation so far, paying close attention to the user's explicit requests and your previous actions.
...
def build_prompt(custom_instructions: str | None = None) -> str:
    """Port of Gse: assemble the trailing synthetic user turn sent after
    the group being summarized."""
    prompt = PREAMBLE + INSTRUCTIONS_BODY
    if custom_instructions and custom_instructions.strip():
        prompt += f"\n\nAdditional Instructions:\n{custom_instructions}"
    return prompt + REMINDER_SUFFIX
